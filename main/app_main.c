...
void LED_Control_Task(void *params)
{
    ...
    while (true)
    {
        if (xQueueReceive(interputQueue1, &pinNumber, portMAX_DELAY))
        {
            ...
            esp_mqtt_client_publish(mqtt_client, "/stu_244/lamp1", data, 0, 0, 0); // <-- publish ไปยัง broker
        }
    }
}
...
void LED2_Control_Task(void *params)
{
    ...
    while (true)
    {
        if (xQueueReceive(interputQueue2, &pinNumber, portMAX_DELAY))
        {
            ...
            esp_mqtt_client_publish(mqtt_client, "/stu_244/lamp2", data, 0, 0, 0); // <-- publish ไปยัง broker
        }
    }
}
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        // // เพิ่มบรรทัดต่อไปนี้ โดยตั้งขื่อเป็น stu-<เลข 3 ตัวท้ายของรหัสนักศึกษา>
         ...
         ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);
         msg_id = esp_mqtt_client_subscribe(client, "/stu_244/lamp2", 0);
         ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);
         msg_id = esp_mqtt_client_subscribe(client, "/stu_244/lamp3", 0);
         ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);
        break;
